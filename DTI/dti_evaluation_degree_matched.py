"""
DTI Evaluation (Binary Classification) - Degree-Matched-Trained Models
=====================================================================

This is the counterpart to dti_evaluation.py, but it loads the KGE
checkpoints that were trained with DEGREE-MATCHED negative sampling
(outputs_*_degree_matched) instead of the uniform-negatives checkpoints.

Note: the *evaluation* negative sampling strategy here is independent of
how the models were trained. By default we still use degree-matched
negatives at evaluation time so the binary-classification setup is
consistent with the training-time assumption.
...
# Model types and their output directory names (degree-matched-trained models)
MODELS = {
    'TransE': 'outputs_transe_degree_matched',
    'ComplEx': 'outputs_complex_degree_matched',
    'TriModel': 'outputs_trimodel_degree_matched',
}
...
class EvalConfig:
    ...
    # Output
    output_dir: str = "outputs_dti_evaluation_degree_matched"
...
def main():
    ...
    print("=" * 70)
    print("DTI EVALUATION (DEGREE-MATCHED-TRAINED MODELS, MULTI-DIMENSION)")
    print("=" * 70)
