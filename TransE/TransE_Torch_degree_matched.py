...
def compute_entity_degrees(triples: torch.Tensor, num_entities: int):
    """
    Count how many times each entity appears as head or tail in the training set.
    Returns a numpy array of shape (num_entities,).
    """
...
def build_degree_bins(degrees: np.ndarray, bin_size: int = 5):
    ...
    Returns:
        bin_of_entity: np.ndarray (num_entities,) bin index of each entity.
        bins:          dict[int, np.ndarray] mapping bin_id -> array of entity ids.
    """
...
def _candidate_pool(
    ...
    """
    Build a candidate entity pool for a given positive bin, expanding to neighbouring
    bins if necessary, falling back to all entities when still too small.
    """
...
def corrupt_triples(
    ...
    """
    Create one degree-matched negative triple for each positive triple by
    corrupting head OR tail with an entity drawn from the same (or a neighbouring)
    degree bin as the entity it replaces.
...
def compute_validation_loss(
    ...
    """
    Compute average margin loss on validation set with degree-matched negatives.
    """
    model.eval()
...
def train_transe(
    ...
    # Degree-matched negative sampling: build entity degrees and bins from training set
...
def save_outputs(output_dir, train_losses, valid_losses, entity2id, relation2id, model: TransE):
    ...
    ax.set_title("TransE (Degree-Matched Negatives) Training & Validation Loss", fontsize=13, fontweight='bold')
...
if __name__ == "__main__":
    ...
    OUTPUT_ROOT = os.path.join(PROJECT_ROOT, "outputs_transe_degree_matched")

    # Reuse the existing splits produced by TransE_Torch.py for a fair comparison
    train_path = os.path.join(DATA_DIR, "train.txt")
    valid_path = os.path.join(DATA_DIR, "valid.txt")
    test_path = os.path.join(DATA_DIR, "test.txt")
    for p in (train_path, valid_path, test_path):
        if not os.path.exists(p):
            raise FileNotFoundError(
                f"Expected split file not found: {p}. Run TransE_Torch.py first to generate splits."
            )
