...
def save_outputs(output_dir, train_losses, valid_losses, entity2id, relation2id, model: ComplEx):
    ...
    ax.set_title("ComplEx (Degree-Matched Negatives) Training & Validation Loss", fontsize=13, fontweight='bold')
...
if __name__ == "__main__":
    ...
    OUTPUT_ROOT = os.path.join(PROJECT_ROOT, "outputs_complex_degree_matched")

    # Reuse the existing splits produced by ComplEx_Torch.py for a fair comparison
    train_path = os.path.join(DATA_DIR, "train.txt")
    valid_path = os.path.join(DATA_DIR, "valid.txt")
    test_path = os.path.join(DATA_DIR, "test.txt")
    for p in (train_path, valid_path, test_path):
        if not os.path.exists(p):
            raise FileNotFoundError(
                f"Expected split file not found: {p}. Run ComplEx_Torch.py first to generate splits."
            )
