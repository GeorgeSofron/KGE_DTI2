"""
TriModel Evaluation Script (Degree-Matched Negatives Variant)
=============================================================
Evaluate a trained TriModel (trained with degree-matched negative sampling)
on the knowledge graph link prediction task.
Computes filtered metrics: MRR, Hits@1, Hits@3, Hits@10, Mean Rank.
"""
...
from utils.model import TriModel  # Shared model definition
...
def compute_ranks_batch(model, test_triples, all_true, num_entities, device, batch_size=256):
    ...
    for start in tqdm(range(0, n_test, batch_size), desc="Evaluating batches"):
        ...
        head_scores = (all_ent_v1 @ r1t3.T +
                       all_ent_v2 @ r2t2.T +
                       all_ent_v3 @ r3t1.T)
        head_scores = head_scores.T.cpu().numpy()
...
def save_results(metrics, output_path):
    ...
    with open(output_path, "w") as f:
        f.write("TriModel (Degree-Matched Negatives) Evaluation Results (Filtered Ranking)\n")
        f.write("=" * 50 + "\n\n")
...
if __name__ == "__main__":
    ...
    # Set up logging: tee stdout/stderr to a run log file
    os.makedirs(OUTPUT_ROOT, exist_ok=True)
    LOG_PATH = os.path.join(
        PROJECT_ROOT,
        f"outputs_trimodel_degree_matched_eval.log",
    )
    _log_file = open(LOG_PATH, "w", encoding="utf-8")
    sys.stdout = TeeLogger(sys.__stdout__, _log_file)
    sys.stderr = TeeLogger(sys.__stderr__, _log_file)
    print(f"Logging evaluation output to: {LOG_PATH}")
    ...
    for dim in EMBEDDING_DIMS:
        ...
        model_path = os.path.join(model_dir, "trimodel_model.pt")
        ...
        print(f"\n{'=' * 60}")
        print(f"Evaluating TriModel (degree-matched) with embedding dimension {dim}")
        ...
        print("\n" + "=" * 50)
        print(f"TriModel (Degree-Matched Negatives) Evaluation Results (Filtered Ranking, dim={dim})")
        print("=" * 50)
        for k, v in metrics.items():
            print(f"{k}: {v:.4f}")
        print(
            f"Eval time: {_format_duration(eval_elapsed)} ({eval_elapsed:.2f}s)"
        )

        save_results(metrics, output_path)
        summary_rows.append({"dimension": dim, **metrics, "eval_time_seconds": eval_elapsed})
    ...
    print(
        f"\nAll evaluations complete. Wall-clock time: "
