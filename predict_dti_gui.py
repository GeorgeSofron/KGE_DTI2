...
class DrugTargetPredictionGUI:
    ...
    def create_widgets(self):
        ...
        # Dimension selector
        ttk.Label(model_frame, text="Dimension:").grid(row=0, column=3, padx=(20, 4))
        self.dim_var = tk.StringVar(value="300")
        dim_combo = ttk.Combobox(model_frame, textvariable=self.dim_var,
                                 values=["100", "200", "300"], width=6, state="readonly")
        dim_combo.grid(row=0, column=4, padx=(0, 10))

        self.load_btn = ttk.Button(model_frame, text="Load Model", command=self.load_model)
        self.load_btn.grid(row=0, column=5, padx=20)

        self.model_status = ttk.Label(model_frame, text="No model loaded", foreground="red")
        self.model_status.grid(row=0, column=6, padx=10)
    ...
    def load_model(self):
        ...
        model_paths = {
            "TransE":   (f"outputs_transe/dim_{dim}/transe_model.pt",   "data/transe/train.txt"),
            "ComplEx":  (f"outputs_complex/dim_{dim}/complex_model.pt",  "data/complex/train.txt"),
            "TriModel": (f"outputs_trimodel/dim_{dim}/trimodel_model.pt", "data/trimodel/train.txt"),
        }

        model_path, train_path = model_paths[model_name]
    ...
    def _load_model_thread(self, model_name, model_path, train_path):
        ...
        try:
            ...
            def update_gui():
                ...
                self.summary_text.insert(tk.END, f"Model: {model_name} (dim={self.dim_var.get()})\n")
