"""
DTI Negative-Strategy Comparison Plot (Random vs Degree-Matched)
================================================================

Compares DTI binary-classification metrics for two evaluation runs:
  - Uniform-Random      -> outputs_dti_evaluation_fixed_random/
                            (negatives sampled uniformly at random)
  - Degree-Matched      -> outputs_dti_evaluation_degree_matched/
                            (negatives matched to target-degree distribution)

Each run already contains a master CSV
    dti_metrics_all_models_dims.csv
with one row per (Model, Dimension).

Generates:
  outputs_dti_negative_strategy_comparison_random/
...
OUT_DIR = os.path.join(PROJECT_ROOT, "outputs_dti_negative_strategy_comparison_random")

MODELS = ["TransE", "ComplEx", "TriModel"]
STRATEGIES = ["Uniform-Random", "Degree-Matched"]
...
SOURCES = {
    "Uniform-Random": os.path.join(PROJECT_ROOT, "outputs_dti_evaluation_fixed_random",
                                    "dti_metrics_all_models_dims.csv"),
...
STRATEGY_LINESTYLE = {"Uniform-Random": "-", "Degree-Matched": "--"}
STRATEGY_MARKER = {"Uniform-Random": "o", "Degree-Matched": "s"}
STRATEGY_HATCH = {"Uniform-Random": "", "Degree-Matched": "//"}
...
def plot_per_metric_lines(df: pd.DataFrame):
    ...
    for metric in METRICS:
        ...
        ax.set_title(f"DTI {metric}: Uniform-Random vs Degree-Matched eval negatives")
        ax.grid(alpha=0.3)
            ...
            for j, model in enumerate(MODELS):
                ...
                for k, strategy in enumerate(STRATEGIES):
                    ...
                    ax.bar(
                        ...
                        color=MODEL_COLOR[model],
                        alpha=0.85 if strategy == "Uniform-Random" else 0.95,
...
def plot_per_model(df: pd.DataFrame):
    ...
    for model in MODELS:
        ...
        fig.suptitle(f"DTI metrics for {model}: Uniform-Random vs Degree-Matched eval negatives", fontsize=13)
...
def plot_per_dim(df: pd.DataFrame):
    ...
    for dim in dims:
        ...
        for ax, metric in zip(axes, METRICS):
            ...
            ax.grid(axis="y", alpha=0.3)
            ax.legend(fontsize=9)
        fig.suptitle(f"DTI metrics at dim={dim}: Uniform-Random vs Degree-Matched eval negatives", fontsize=13)
        plt.tight_layout(rect=[0, 0, 1, 0.96])
